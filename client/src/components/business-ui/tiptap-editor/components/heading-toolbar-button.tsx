...
const HEADING_OPTIONS = [
  { level: 0, label: '正文', icon: 'T' },
  { level: 1, label: '一级标题', icon: 'H1' },
  { level: 2, label: '二级标题', icon: 'H2' },
  { level: 3, label: '三级标题', icon: 'H3' },
  { level: 4, label: '四级标题', icon: 'H4' },
  { level: 5, label: '五级标题', icon: 'H5' },
  { level: 6, label: '六级标题', icon: 'H6' },
] as const;
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="sm" className="h-6 gap-0.5 px-2">
          <span className="inline-flex items-center justify-center text-base leading-none">{ActiveIcon}</span>
